...
// Simple check if we're inside Ema frame by checking DOM
export function useIsInFrame() {
  ...
  useEffect(() => {
    ...
    const checkFrame = () => {
      // Check if .ema-frame-mobile exists in DOM
      const frameElement = document.querySelector('.ema-frame-mobile')
      setIsInFrame(!!frameElement)
    }

    checkFrame()

    // Watch for changes
    const observer = new MutationObserver(checkFrame)
...
export function useScrollRestoration(router, delay) {
  ...
  const updateScrollPosition = (url, restore, shouldRestore) => {
    ...
    // Check if we have a scroll position from chat navigation (sessionStorage)
    const chatRestoreScroll = typeof window !== 'undefined' 
      ? sessionStorage.getItem('emaChatRestoreScroll') 
      : null
    const chatRestoreRoute = typeof window !== 'undefined'
      ? sessionStorage.getItem('emaChatRestoreRoute')
      : null

    // If we have a chat restore position for this route, use it instead
    let scrollPosition = position && shouldRestore ? position : 0
    if (chatRestoreScroll && chatRestoreRoute) {
      const currentPath = url.split('?')[0]
      const restorePath = chatRestoreRoute.split('?')[0]
      if (currentPath === restorePath) {
        scrollPosition = parseInt(chatRestoreScroll, 10)
        // Clear the flags since we're using them
        sessionStorage.removeItem('emaChatRestoreScroll')
        sessionStorage.removeItem('emaChatRestoreRoute')
      }
    }
...
/*  Image helpers
/*  ------------------------------ */
// Hotspot/crop support enabled for all images
...
export function buildSrc(image, { width, height, format, quality }) {
  ...
  // Use crop fit mode when hotspot/crop data exists, otherwise use max for backward compatibility
  // Sanity's image builder automatically uses hotspot coordinates when fit('crop') is called
  const hasHotspot = image && (image.hotspot || image.crop)
  const fitMode = hasHotspot ? 'crop' : 'max'
  
  return imgSrc.fit(fitMode).auto('format').url()
}

export function buildSrcSet(image, { srcSizes = [400, 600, 800, 1000, 1200], aspect, format, quality } = {}) {
  if (!Array.isArray(srcSizes)) {
    console.warn('buildSrcSet received an invalid srcSizes:', srcSizes);
    srcSizes = [400, 600, 800, 1000, 1200]; // Fallback
  }
...
/**
 * Builds a CSS gradient string from a gradient object
 * @param {Object} gradient - Gradient object from Sanity
 * @param {string} gradient.type - 'linear' or 'radial'
 * @param {string} gradient.direction - Direction for linear gradients (e.g., 'to bottom')
 * @param {string} gradient.customAngle - Custom angle in degrees (e.g., '45deg')
 * @param {Array} gradient.colorStops - Array of color stop objects with color and position
 * @returns {string|null} CSS gradient string or null if invalid
 */
export function buildGradientString(gradient) {
  if (!gradient || !gradient.colorStops || gradient.colorStops.length < 2) {
    return null
  }

  const { type, direction, customAngle, colorStops } = gradient
...
/**
 * Builds a style object with gradient background from a gradient object
 * @param {Object} gradient - Gradient object from Sanity
 * @param {Object} additionalStyles - Additional styles to merge
 * @returns {Object} Style object with background property
 */
export function buildGradientStyle(gradient, additionalStyles = {}) {
  const gradientString = buildGradientString(gradient)
